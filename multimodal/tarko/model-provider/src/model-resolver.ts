...
export function resolveModel(
  ...
  // AWS Bedrock support
  const region = agentModel?.region;
  const accessKeyId = agentModel?.accessKeyId;
  const secretAccessKey = agentModel?.secretAccessKey;
  const sessionToken = agentModel?.sessionToken;
