...
得益于 Agent TARS 新版本中引入的 [Agent Event Stream](#agent-event-stream)，Web UI 能够完全独立地进行开发，并通过协议与 [Agent TARS Server](../guide/advanced/server.mdx) 交互，最终，我们带来了纯净的 Web UI：
